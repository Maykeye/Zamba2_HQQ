...
class HybridMambaAttentionDynamicCache:
    def __init__(self, config, batch_size, dtype=torch.bfloat16, device=None):
        ...
        self.key_cache = [torch.tensor(
            [[]] * batch_size, device=device) for _ in range(config.num_hidden_layers)]
    ...
    def reorder_cache(self, beam_idx: torch.LongTensor):
        ...
        for layer_idx in range(len(self.key_cache)):
            ...
            self.key_value_memory_dict_mamba[layer_idx] = (
...
class Zamba2Attention(nn.Module):
    ...
    def __init__(self, config: Zamba2Config, layer_idx: Optional[int] = None, num_mem_blocks=None):
        ...
        self.q_proj = nn.Linear(2 * self.hidden_size,
                                self.num_heads * self.head_dim, bias=False)
        self.k_proj = nn.Linear(
            2 * self.hidden_size, self.num_key_value_heads * self.head_dim, bias=False)
        ...
        if config.use_shared_attention_lora:
            ...
            for i in range(self.num_mem_blocks):
                ...
                linear_q_lora_B = nn.Linear(
                    self.config.lora_rank, 2 * self.config.hidden_size, bias=False)
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output = attn_output.reshape(
            bsz, q_len, 2 * self.hidden_size).contiguous()
        attn_output = self.o_proj(attn_output)
...
class Zamba2AttentionDecoderLayer(nn.Module):
    def __init__(self, config: Zamba2Config, layer_idx: Optional[int] = None):
        ...
        self.input_layernorm = Zamba2RMSNorm(
            2 * config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            hidden_states, attention_mask=attention_mask, past_key_value=past_key_value, position_ids=position_ids, layer_idx=layer_idx)

        # feed-forward (MLP)
        hidden_states = self.pre_ff_layernorm(hidden_states)
        hidden_states = self.feed_forward(
            hidden_states, forward_layer_idx=layer_idx)
...
class Zamba2MambaDecoderLayer(nn.Module):
    def __init__(self, config: Zamba2Config, layer_idx: int):
        ...
        self.mamba = Mamba2Layer(
            config=config, layer_idx=layer_idx, **factory_kwargs)
        self.input_layernorm = Zamba2RMSNorm(
            config.hidden_size, eps=config.rms_norm_eps)
        self.layer_idx = layer_idx
...
class Zamba2PreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = [
        "Zamba2AttentionDecoderLayer", "Zamba2MambaDecoderLayer"]
...
class Zamba2Model(Zamba2PreTrainedModel):
    ...
    def __init__(self, config: Zamba2Config):
        ...
        self.blocks = torch.nn.ModuleList(
            [Zamba2AttentionDecoderLayer(config) for _ in range(config.num_mem_blocks)])
        mamba_layers = []
        linear_layers = []
        self.layers_block_type = config.layers_block_type
        for i in range(config.num_hidden_layers):
            if config.layers_block_type[i] == "m":
                mamba_layers.append(
                    Zamba2MambaDecoderLayer(config, layer_idx=i))
                ...
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(
                        self.blocks[block_count %
                                    self.config.num_mem_blocks].__call__,
                        hidden_states,
