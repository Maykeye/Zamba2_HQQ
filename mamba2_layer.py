...
class Mamba2Layer(nn.Module):
    def __init__(
        ...
        self.in_proj = nn.ModuleList([nn.Linear(
            self.d_model, d_in_proj, bias=self.config.add_bias_linear, **factory_kwargs)])
        ...
        A = torch.empty(self.nheads, dtype=torch.float32).uniform_(
            *A_init_range)
        ...
        self.out_proj = nn.Linear(
            self.d_inner, self.d_model, bias=self.config.add_bias_linear, **factory_kwargs)

        if self.config.ft_lora:
            self.x_lora_A = nn.Linear(
                self.d_inner, self.config.lora_rank, bias=False)
            self.x_lora_B = nn.Linear(
                self.config.lora_rank, self.d_inner, bias=False)
            nn.init.zeros_(self.x_lora_B.weight)
            self.z_lora_A = nn.Linear(
                self.d_inner, self.config.lora_rank, bias=False)
            self.z_lora_B = nn.Linear(
                self.config.lora_rank, self.d_inner, bias=False)
            nn.init.zeros_(self.z_lora_B.weight)
            self.out_proj_lora_A = nn.Linear(
                self.d_inner, self.config.lora_rank, bias=False)
            self.out_proj_lora_B = nn.Linear(
                self.config.lora_rank, self.d_model, bias=False)
    ...
    def forward(self,
        ...
        dt_limit_kwargs = {} if self.dt_limit == (
            0.0, float("inf")) else dict(dt_limit=self.dt_limit)
        ...
        if self.use_mem_eff_path and inference_params is None and input_not_masked and not self.config.ft_lora:
            out = mamba_split_conv1d_scan_combined(
                ...
                self.dt_bias,
                A,
                D=rearrange(self.D, "(h p) -> h p",
                            p=self.headdim) if self.D_has_hdim else self.D,
            ...
            d_mlp = (zxbcdt.shape[-1] - 2 * self.d_ssm - 2 *
                     self.ngroups * self.d_state - self.nheads) // 2
            z0, x0, z, xBC, dt = torch.split(
                zxbcdt,
                [d_mlp, d_mlp, self.d_ssm, self.d_ssm + 2 *
            ...
            y = mamba_chunk_scan_combined(
                ...
                z=rearrange(z, "b l (h p) -> b l h p",
                            p=self.headdim) if not self.rmsnorm else None,
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        if selective_state_update is None:
            ...
            A = repeat(A, "h -> h p n", p=self.headdim,
